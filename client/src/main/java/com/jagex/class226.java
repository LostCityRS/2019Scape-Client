package com.jagex;

import com.jagex.deob.ObfuscatedName;
import com.jagex.deob.Statics;

@ObfuscatedName("hf")
public class class226 {

	@ObfuscatedName("hf.e")
	public static int field2234 = 0;

	@ObfuscatedName("hf.n")
	public static int field2301 = 1;

	@ObfuscatedName("hf.m")
	public static int field2157 = 2;

	@ObfuscatedName("hf.k")
	public static int field2266 = 3;

	@ObfuscatedName("hf.f")
	public static int field2159 = 328;

	@ObfuscatedName("hf.w")
	public static int field2341 = 1337;

	@ObfuscatedName("hf.l")
	public static int field2161 = 1403;

	@ObfuscatedName("hf.u")
	public static int field2171 = 1338;

	@ObfuscatedName("hf.z")
	public static int field2259 = 1339;

	@ObfuscatedName("hf.p")
	public static int field2164 = 1400;

	@ObfuscatedName("hf.d")
	public static int field2165 = 1401;

	@ObfuscatedName("hf.c")
	public static int field2166 = 1405;

	@ObfuscatedName("hf.r")
	public static int field2167 = 1406;

	@ObfuscatedName("hf.v")
	public static int field2168 = 1407;

	@ObfuscatedName("hf.o")
	public static int field2169 = 1408;

	@ObfuscatedName("hf.s")
	public static int field2340 = 1409;

	@ObfuscatedName("hf.y")
	public static int field2163 = 1410;

	@ObfuscatedName("hf.q")
	public static int field2172 = 1411;

	@ObfuscatedName("hf.t")
	public static class162 field2175 = new class162(6000000, 200);

	@ObfuscatedName("hf.ae")
	public static class162 field2176 = new class162(8);

	@ObfuscatedName("hf.ag")
	public static class162 field2177 = new class162(4);

	@ObfuscatedName("hf.ah")
	public static class162 field2297 = new class162(50);

	@ObfuscatedName("hf.ac")
	public static boolean field2338 = false;

	@ObfuscatedName("hf.ai")
	public String field2155;

	@ObfuscatedName("hf.aw")
	public int field2181;

	@ObfuscatedName("hf.as")
	public int field2158 = -1;

	@ObfuscatedName("hf.at")
	public int field2183 = -1;

	@ObfuscatedName("hf.ad")
	public int field2184;

	@ObfuscatedName("hf.am")
	public int field2185 = 0;

	@ObfuscatedName("hf.au")
	public byte field2186 = 0;

	@ObfuscatedName("hf.ar")
	public byte field2187 = 0;

	@ObfuscatedName("hf.ap")
	public byte field2356 = 0;

	@ObfuscatedName("hf.aq")
	public byte field2174 = 0;

	@ObfuscatedName("hf.ax")
	public int field2190 = 0;

	@ObfuscatedName("hf.av")
	public int field2191 = 0;

	@ObfuscatedName("hf.ao")
	public int field2192 = 0;

	@ObfuscatedName("hf.aj")
	public int field2193 = 0;

	@ObfuscatedName("hf.ay")
	public int field2194 = 0;

	@ObfuscatedName("hf.ab")
	public int field2195 = 0;

	@ObfuscatedName("hf.az")
	public int field2196 = 0;

	@ObfuscatedName("hf.aa")
	public int field2197 = 0;

	@ObfuscatedName("hf.af")
	public int field2198 = 1;

	@ObfuscatedName("hf.ak")
	public int field2199 = 1;

	@ObfuscatedName("hf.an")
	public int field2200 = -1;

	@ObfuscatedName("hf.bf")
	public boolean field2201 = false;

	@ObfuscatedName("hf.bl")
	public int field2202 = -1;

	@ObfuscatedName("hf.bk")
	public int field2269 = -1;

	@ObfuscatedName("hf.bh")
	public int field2204 = -1;

	@ObfuscatedName("hf.bx")
	public boolean field2205 = false;

	@ObfuscatedName("hf.bd")
	public int field2206 = 0;

	@ObfuscatedName("hf.bc")
	public int field2207 = 0;

	@ObfuscatedName("hf.bi")
	public int field2208 = 0;

	@ObfuscatedName("hf.bn")
	public int field2319 = 0;

	@ObfuscatedName("hf.bt")
	public int field2210 = 0;

	@ObfuscatedName("hf.bq")
	public boolean field2263 = false;

	@ObfuscatedName("hf.bm")
	public int field2212 = 0;

	@ObfuscatedName("hf.bb")
	public int field2156 = 1;

	@ObfuscatedName("hf.be")
	public boolean field2214 = false;

	@ObfuscatedName("hf.by")
	public int field2215 = -1;

	@ObfuscatedName("hf.bu")
	public int field2216 = 0;

	@ObfuscatedName("hf.bw")
	public boolean field2217 = false;

	@ObfuscatedName("hf.bo")
	public int field2218 = 0;

	@ObfuscatedName("hf.bz")
	public int field2219 = 0;

	@ObfuscatedName("hf.bv")
	public boolean field2220;

	@ObfuscatedName("hf.br")
	public boolean field2213;

	@ObfuscatedName("hf.bg")
	public boolean field2222 = false;

	@ObfuscatedName("hf.ba")
	public boolean field2253 = true;

	@ObfuscatedName("hf.bp")
	public int field2224 = 1;

	@ObfuscatedName("hf.bj")
	public int field2294;

	@ObfuscatedName("hf.bs")
	public int field2298 = -1;

	@ObfuscatedName("hf.cl")
	public boolean field2274;

	@ObfuscatedName("hf.cg")
	public int field2228 = 0;

	@ObfuscatedName("hf.ce")
	public int field2316 = 0;

	@ObfuscatedName("hf.cu")
	public int field2230 = 0;

	@ObfuscatedName("hf.ci")
	public int field2231 = 0;

	@ObfuscatedName("hf.cn")
	public int field2331 = 0;

	@ObfuscatedName("hf.cv")
	public int field2305 = 0;

	@ObfuscatedName("hf.cp")
	public int field2209 = 0;

	@ObfuscatedName("hf.ca")
	public int field2235 = 0;

	@ObfuscatedName("hf.cx")
	public int field2272 = 100;

	@ObfuscatedName("hf.cw")
	public int field2227 = 0;

	@ObfuscatedName("hf.ct")
	public int field2238 = 0;

	@ObfuscatedName("hf.cf")
	public boolean field2239 = false;

	@ObfuscatedName("hf.co")
	public boolean field2260 = false;

	@ObfuscatedName("hf.cr")
	public int field2241 = 2;

	@ObfuscatedName("hf.cm")
	public short[] field2242;

	@ObfuscatedName("hf.cq")
	public short[] field2243;

	@ObfuscatedName("hf.ch")
	public short[] field2346;

	@ObfuscatedName("hf.cb")
	public short[] field2245;

	@ObfuscatedName("hf.cs")
	public int field2240;

	@ObfuscatedName("hf.cy")
	public int field2256;

	@ObfuscatedName("hf.cc")
	public int field2203;

	@ObfuscatedName("hf.cz")
	public int field2189;

	@ObfuscatedName("hf.ck")
	public boolean field2250 = false;

	@ObfuscatedName("hf.cj")
	public int field2345;

	@ObfuscatedName("hf.cd")
	public int field2252;

	@ObfuscatedName("hf.dd")
	public int field2302;

	@ObfuscatedName("hf.dr")
	public int field2251;

	@ObfuscatedName("hf.da")
	public int field2255;

	@ObfuscatedName("hf.dt")
	public int field2248;

	@ObfuscatedName("hf.do")
	public int field2257;

	@ObfuscatedName("hf.dz")
	public int field2258;

	@ObfuscatedName("hf.dv")
	public int field2287 = -1;

	@ObfuscatedName("hf.dm")
	public boolean field2275 = true;

	@ObfuscatedName("hf.dq")
	public String field2261 = "";

	@ObfuscatedName("hf.dc")
	public int field2229 = 0;

	@ObfuscatedName("hf.di")
	public int field2223 = 0;

	@ObfuscatedName("hf.dk")
	public int field2264 = 0;

	@ObfuscatedName("hf.dn")
	public boolean field2265 = false;

	@ObfuscatedName("hf.df")
	public int field2236 = 0;

	@ObfuscatedName("hf.dw")
	public boolean field2267 = false;

	@ObfuscatedName("hf.ds")
	public class976 field2268 = class976.field11386;

	@ObfuscatedName("hf.du")
	public boolean field2324 = false;

	@ObfuscatedName("hf.dl")
	public byte[][] field2270;

	@ObfuscatedName("hf.dp")
	public byte[][] field2281;

	@ObfuscatedName("hf.dy")
	public int[] field2211;

	@ObfuscatedName("hf.db")
	public int[] field2273;

	@ObfuscatedName("hf.dh")
	public int[] field2300;

	@ObfuscatedName("hf.eo")
	public String field2279 = "";

	@ObfuscatedName("hf.ey")
	public String[] field2321;

	@ObfuscatedName("hf.eu")
	public String field2180;

	@ObfuscatedName("hf.ed")
	public int[] field2282;

	@ObfuscatedName("hf.ee")
	public int field2283 = -1;

	@ObfuscatedName("hf.es")
	public class226 field2284 = null;

	@ObfuscatedName("hf.ei")
	public int field2285 = 0;

	@ObfuscatedName("hf.el")
	public int field2286 = 0;

	@ObfuscatedName("hf.ej")
	public int field2271 = field2234;

	@ObfuscatedName("hf.ep")
	public String field2288 = "";

	@ObfuscatedName("hf.ev")
	public boolean field2179 = false;

	@ObfuscatedName("hf.ec")
	public Object[] field2290;

	@ObfuscatedName("hf.ek")
	public Object[] field2291;

	@ObfuscatedName("hf.em")
	public Object[] field2292;

	@ObfuscatedName("hf.eh")
	public Object[] field2293;

	@ObfuscatedName("hf.eq")
	public Object[] field2244;

	@ObfuscatedName("hf.eg")
	public Object[] field2233;

	@ObfuscatedName("hf.ez")
	public Object[] field2296;

	@ObfuscatedName("hf.ef")
	public Object[] field2280;

	@ObfuscatedName("hf.et")
	public Object[] field2362;

	@ObfuscatedName("hf.ea")
	public Object[] field2178;

	@ObfuscatedName("hf.ew")
	public Object[] field2249;

	@ObfuscatedName("hf.er")
	public Object[] field2299;

	@ObfuscatedName("hf.en")
	public Object[] field2322;

	@ObfuscatedName("hf.eb")
	public int[] field2303;

	@ObfuscatedName("hf.ex")
	public Object[] field2304;

	@ObfuscatedName("hf.fg")
	public int[] field2247;

	@ObfuscatedName("hf.fm")
	public Object[] field2306;

	@ObfuscatedName("hf.fu")
	public int[] field2307;

	@ObfuscatedName("hf.fs")
	public Object[] field2308;

	@ObfuscatedName("hf.fz")
	public int[] field2309;

	@ObfuscatedName("hf.fj")
	public Object[] field2310;

	@ObfuscatedName("hf.fd")
	public int[] field2311;

	@ObfuscatedName("hf.fn")
	public Object[] field2312;

	@ObfuscatedName("hf.fi")
	public int[] field2313;

	@ObfuscatedName("hf.ft")
	public Object[] field2314;

	@ObfuscatedName("hf.fx")
	public Object[] field2315;

	@ObfuscatedName("hf.fv")
	public Object[] field2225;

	@ObfuscatedName("hf.fc")
	public Object[] field2317;

	@ObfuscatedName("hf.fw")
	public Object[] field2318;

	@ObfuscatedName("hf.fa")
	public Object[] field2295;

	@ObfuscatedName("hf.fp")
	public Object[] field2320;

	@ObfuscatedName("hf.fq")
	public Object[] field2355;

	@ObfuscatedName("hf.ff")
	public Object[] field2232;

	@ObfuscatedName("hf.fl")
	public Object[] field2323;

	@ObfuscatedName("hf.fb")
	public Object[] field2347;

	@ObfuscatedName("hf.fo")
	public Object[] field2325;

	@ObfuscatedName("hf.fy")
	public Object[] field2326;

	@ObfuscatedName("hf.fe")
	public Object[] field2327;

	@ObfuscatedName("hf.fk")
	public Object[] field2328;

	@ObfuscatedName("hf.fh")
	public Object[] field2329;

	@ObfuscatedName("hf.fr")
	public Object[] field2330;

	@ObfuscatedName("hf.gu")
	public Object[] field2359;

	@ObfuscatedName("hf.gq")
	public Object[] field2332;

	@ObfuscatedName("hf.gl")
	public class729 field2333;

	@ObfuscatedName("hf.go")
	public int field2334 = -1;

	@ObfuscatedName("hf.gk")
	public int field2335 = 0;

	@ObfuscatedName("hf.gp")
	public String field2336 = null;

	@ObfuscatedName("hf.gy")
	public class223 field2337;

	@ObfuscatedName("hf.ga")
	public boolean field2246 = false;

	@ObfuscatedName("hf.gn")
	public int field2339 = -1;

	@ObfuscatedName("hf.gc")
	public int field2348;

	@ObfuscatedName("hf.gf")
	public int field2162;

	@ObfuscatedName("hf.gx")
	public int field2342;

	@ObfuscatedName("hf.ge")
	public int field2343;

	@ObfuscatedName("hf.gg")
	public int field2344;

	@ObfuscatedName("hf.gr")
	public int field2188 = -1;

	@ObfuscatedName("hf.gs")
	public int field2262 = -1;

	@ObfuscatedName("hf.gt")
	public class721 field2170;

	@ObfuscatedName("hf.gh")
	public class226[] field2351;

	@ObfuscatedName("hf.gm")
	public class226[] field2349;

	@ObfuscatedName("hf.gv")
	public boolean field2350 = false;

	@ObfuscatedName("hf.gj")
	public boolean field2360 = false;

	@ObfuscatedName("hf.gw")
	public int field2352 = -1;

	@ObfuscatedName("hf.gd")
	public int field2353 = 0;

	@ObfuscatedName("hf.gz")
	public int field2354 = 0;

	@ObfuscatedName("hf.gb")
	public int field2160 = 0;

	@ObfuscatedName("hf.gi")
	public int field2289 = 0;

	@ObfuscatedName("hf.hm")
	public int field2357 = 0;

	@ObfuscatedName("hf.ha")
	public int field2358 = 0;

	@ObfuscatedName("hf.hk")
	public int field2182 = -1;

	@ObfuscatedName("hf.hu")
	public int field2237 = -1;

	@ObfuscatedName("hf.hb")
	public int[] field2361;

	@ObfuscatedName("hf.hl")
	public class634 field2254;

	@ObfuscatedName("hf.hg")
	public class259 field2363;

	@ObfuscatedName("gb.e(Lpy;Lpy;Lpy;Lpy;I)V")
	public static void method3669(class442 arg0, class442 arg1, class442 arg2, class442 arg3) {
		Statics.field7085 = arg0;
		Statics.field1707 = arg1;
		Statics.field8370 = arg2;
		Statics.field11725 = new class225[Statics.field7085.method6898()];
		Statics.field8424 = new boolean[Statics.field7085.method6898()];
	}

	@ObfuscatedName("xh.n(II)Lhf;")
	public static class226 method10202(int arg0) {
		int var1 = arg0 >> 16;
		if (Statics.field11725[var1] == null || Statics.field11725[var1].method3924(arg0) == null) {
			boolean var2 = method5364(var1, null);
			if (!var2) {
				return null;
			}
		}
		return Statics.field11725[var1].method3924(arg0);
	}

	@ObfuscatedName("ahh.k(IIB)Lhf;")
	public static class226 method16682(int arg0, int arg1) {
		class226 var2 = method10202(arg0);
		if (arg1 == -1) {
			return var2;
		} else if (var2 == null || var2.field2351 == null || arg1 >= var2.field2351.length) {
			return null;
		} else {
			return var2.field2351[arg1];
		}
	}

	@ObfuscatedName("kn.f(I[II)Z")
	public static boolean method5364(int arg0, int[] arg1) {
		if (Statics.field8424[arg0]) {
			return true;
		}
		Statics.field11725[arg0] = method5405(arg0, arg1, Statics.field11725[arg0], false);
		if (Statics.field11725[arg0] == null) {
			return false;
		} else {
			Statics.field8424[arg0] = true;
			return true;
		}
	}

	@ObfuscatedName("la.w(I[ILhq;ZI)Lhq;")
	public static class225 method5405(int arg0, int[] arg1, class225 arg2, boolean arg3) {
		if (!Statics.field7085.method6927(arg0)) {
			return null;
		}
		int var4 = Statics.field7085.method6897(arg0);
		class226[] var5;
		if (var4 == 0) {
			var5 = new class226[0];
		} else if (arg2 == null) {
			var5 = new class226[var4];
		} else {
			var5 = arg2.field2151;
		}
		if (arg2 == null) {
			arg2 = new class225(arg3, var5);
		} else {
			arg2.field2151 = var5;
			arg2.field2150 = arg3;
		}
		for (int var6 = 0; var6 < var4; var6++) {
			if (arg2.field2151[var6] == null) {
				byte[] var7 = Statics.field7085.method6914(arg0, var6, arg1);
				if (var7 != null) {
					class226 var8 = arg2.field2151[var6] = new class226();
					var8.field2158 = (arg0 << 16) + var6;
					var8.method3938(new class997(var7));
				}
			}
		}
		return arg2;
	}

	@ObfuscatedName("ahr.l(II)V")
	public static void method16640(int arg0) {
		if (arg0 != -1 && !Statics.field8424[arg0]) {
			Statics.field7085.method6899(arg0);
			Statics.field11725[arg0] = null;
		}
	}

	@ObfuscatedName("rq.u(II)V")
	public static void method7602(int arg0) {
		Statics.field8424[arg0] = false;
		method16640(arg0);
	}

	@ObfuscatedName("hf.z(Lalw;I)V")
	public void method3938(class997 arg0) {
		int var2 = arg0.method17904();
		if (var2 == 255) {
			var2 = -1;
		}
		this.field2184 = arg0.method17904();
		if ((this.field2184 & 0x80) != 0) {
			this.field2184 &= 0x7F;
			this.field2155 = arg0.method17918();
		}
		this.field2185 = arg0.method17906();
		this.field2190 = arg0.method17973();
		this.field2191 = arg0.method17973();
		this.field2192 = arg0.method17906();
		this.field2193 = arg0.method17906();
		this.field2356 = arg0.method17905();
		this.field2174 = arg0.method17905();
		this.field2186 = arg0.method17905();
		this.field2187 = arg0.method17905();
		if (this.field2356 == 4 || this.field2174 == 4) {
			this.field2198 = arg0.method17906();
			this.field2199 = arg0.method17906();
		}
		this.field2200 = arg0.method17906();
		if (this.field2200 == 65535) {
			this.field2200 = -1;
		} else {
			this.field2200 += this.field2158 & 0xFFFF0000;
		}
		int var3 = arg0.method17904();
		this.field2201 = (var3 & 0x1) != 0;
		if (var2 >= 0) {
			this.field2205 = (var3 & 0x2) != 0;
		}
		if (this.field2184 == 0) {
			this.field2208 = arg0.method17906();
			this.field2319 = arg0.method17906();
			if (var2 < 0) {
				this.field2205 = arg0.method17904() == 1;
			}
		}
		if (this.field2184 == 5) {
			this.field2215 = arg0.method17910();
			this.field2216 = arg0.method17906();
			int var4 = arg0.method17904();
			this.field2217 = (var4 & 0x1) != 0;
			this.field2222 = (var4 & 0x2) != 0;
			this.field2212 = arg0.method17904();
			this.field2218 = arg0.method17904();
			this.field2219 = arg0.method17910();
			this.field2220 = arg0.method17904() == 1;
			this.field2213 = arg0.method17904() == 1;
			this.field2210 = arg0.method17910();
			if (var2 >= 3) {
				this.field2253 = arg0.method17904() == 1;
			}
		}
		if (this.field2184 == 6) {
			this.field2224 = 1;
			this.field2294 = arg0.method17927();
			int var5 = arg0.method17904();
			boolean var6 = (var5 & 0x1) == 1;
			this.field2274 = (var5 & 0x2) == 2;
			this.field2239 = (var5 & 0x4) == 4;
			this.field2260 = (var5 & 0x8) == 8;
			if (var6) {
				this.field2305 = arg0.method17973();
				this.field2209 = arg0.method17973();
				this.field2230 = arg0.method17906();
				this.field2231 = arg0.method17906();
				this.field2331 = arg0.method17906();
				this.field2272 = arg0.method17906();
			} else if (this.field2274) {
				this.field2305 = arg0.method17973();
				this.field2209 = arg0.method17973();
				this.field2235 = arg0.method17973();
				this.field2230 = arg0.method17906();
				this.field2231 = arg0.method17906();
				this.field2331 = arg0.method17906();
				this.field2272 = arg0.method17973();
			}
			this.field2262 = arg0.method17927();
			if (this.field2356 != 0) {
				this.field2227 = arg0.method17906();
			}
			if (this.field2174 != 0) {
				this.field2238 = arg0.method17906();
			}
		}
		if (this.field2184 == 4) {
			this.field2287 = arg0.method17927();
			if (var2 >= 2) {
				this.field2275 = arg0.method17904() == 1;
			}
			this.field2261 = arg0.method17918();
			this.field2229 = arg0.method17904();
			this.field2223 = arg0.method17904();
			this.field2264 = arg0.method17904();
			this.field2265 = arg0.method17904() == 1;
			this.field2210 = arg0.method17910();
			this.field2212 = arg0.method17904();
			if (var2 >= 0) {
				this.field2236 = arg0.method17904();
			}
		}
		if (this.field2184 == 3) {
			this.field2210 = arg0.method17910();
			this.field2263 = arg0.method17904() == 1;
			this.field2212 = arg0.method17904();
		}
		if (this.field2184 == 9) {
			this.field2156 = arg0.method17904();
			this.field2210 = arg0.method17910();
			this.field2214 = arg0.method17904() == 1;
		}
		int var7 = arg0.method17908();
		int var8 = arg0.method17904();
		if (var8 != 0) {
			this.field2270 = new byte[11][];
			this.field2281 = new byte[11][];
			this.field2211 = new int[11];
			this.field2300 = new int[11];
			while (var8 != 0) {
				int var9 = (var8 >> 4) - 1;
				int var10 = var8 << 8 | arg0.method17904();
				int var11 = var10 & 0xFFF;
				if (var11 == 4095) {
					var11 = -1;
				}
				byte var12 = arg0.method17905();
				if (var12 != 0) {
					this.field2324 = true;
				}
				byte var13 = arg0.method17905();
				this.field2211[var9] = var11;
				this.field2270[var9] = new byte[] { var12 };
				this.field2281[var9] = new byte[] { var13 };
				var8 = arg0.method17904();
			}
		}
		this.field2279 = arg0.method17918();
		int var14 = arg0.method17904();
		int var15 = var14 & 0xF;
		int var16 = var14 >> 4;
		if (var15 > 0) {
			this.field2321 = new String[var15];
			for (int var17 = 0; var17 < var15; var17++) {
				this.field2321[var17] = arg0.method17918();
			}
		}
		if (var16 > 0) {
			int var18 = arg0.method17904();
			this.field2282 = new int[var18 + 1];
			for (int var19 = 0; var19 < this.field2282.length; var19++) {
				this.field2282[var19] = -1;
			}
			this.field2282[var18] = arg0.method17906();
		}
		if (var16 > 1) {
			int var20 = arg0.method17904();
			this.field2282[var20] = arg0.method17906();
		}
		this.field2180 = arg0.method17918();
		if (this.field2180.equals("")) {
			this.field2180 = null;
		}
		this.field2285 = arg0.method17904();
		this.field2286 = arg0.method17904();
		this.field2271 = arg0.method17904();
		this.field2288 = arg0.method17918();
		int var21 = -1;
		if (class976.method16667(var7) != 0) {
			var21 = arg0.method17906();
			if (var21 == 65535) {
				var21 = -1;
			}
			this.field2202 = arg0.method17906();
			if (this.field2202 == 65535) {
				this.field2202 = -1;
			}
			this.field2269 = arg0.method17906();
			if (this.field2269 == 65535) {
				this.field2269 = -1;
			}
		}
		if (var2 >= 0) {
			this.field2204 = arg0.method17906();
			if (this.field2204 == 65535) {
				this.field2204 = -1;
			}
		}
		this.field2268 = new class976(var7, var21);
		if (var2 >= 0) {
			int var22 = arg0.method17904();
			for (int var23 = 0; var23 < var22; var23++) {
				int var24 = arg0.method17908();
				int var25 = arg0.method17910();
				this.field2333.method14501(new class988(var25), (long) var24);
			}
			int var26 = arg0.method17904();
			for (int var27 = 0; var27 < var26; var27++) {
				int var28 = arg0.method17908();
				String var29 = arg0.method17919();
				this.field2333.method14501(new class984(var29), (long) var28);
			}
		}
		this.field2290 = this.method3939(arg0);
		this.field2233 = this.method3939(arg0);
		this.field2280 = this.method3939(arg0);
		this.field2299 = this.method3939(arg0);
		this.field2249 = this.method3939(arg0);
		this.field2322 = this.method3939(arg0);
		this.field2304 = this.method3939(arg0);
		this.field2306 = this.method3939(arg0);
		this.field2314 = this.method3939(arg0);
		this.field2315 = this.method3939(arg0);
		if (var2 >= 0) {
			this.field2225 = this.method3939(arg0);
		}
		this.field2296 = this.method3939(arg0);
		this.field2291 = this.method3939(arg0);
		this.field2292 = this.method3939(arg0);
		this.field2293 = this.method3939(arg0);
		this.field2244 = this.method3939(arg0);
		this.field2362 = this.method3939(arg0);
		this.field2178 = this.method3939(arg0);
		this.field2317 = this.method3939(arg0);
		this.field2308 = this.method3939(arg0);
		this.field2310 = this.method3939(arg0);
		this.field2303 = this.method3940(arg0);
		this.field2247 = this.method3940(arg0);
		this.field2307 = this.method3940(arg0);
		this.field2309 = this.method3940(arg0);
		this.field2311 = this.method3940(arg0);
	}

	@ObfuscatedName("hf.p(Lalw;I)[Ljava/lang/Object;")
	public Object[] method3939(class997 arg0) {
		int var2 = arg0.method17904();
		if (var2 == 0) {
			return null;
		}
		Object[] var3 = new Object[var2];
		for (int var4 = 0; var4 < var2; var4++) {
			int var5 = arg0.method17904();
			if (var5 == 0) {
				var3[var4] = Integer.valueOf(arg0.method17910());
			} else if (var5 == 1) {
				var3[var4] = arg0.method17918();
			}
		}
		this.field2179 = true;
		return var3;
	}

	@ObfuscatedName("hf.d(Lalw;B)[I")
	public int[] method3940(class997 arg0) {
		int var2 = arg0.method17904();
		if (var2 == 0) {
			return null;
		}
		int[] var3 = new int[var2];
		for (int var4 = 0; var4 < var2; var4++) {
			var3[var4] = arg0.method17910();
		}
		return var3;
	}

	@ObfuscatedName("hf.c(Ldh;B)Lcm;")
	public class101 method3941(class133 arg0) {
		field2338 = false;
		long var2 = ((long) this.field2219 << 40) + ((long) this.field2218 << 36) + (long) this.field2215 + ((this.field2222 ? 1L : 0L) << 35) + ((this.field2220 ? 1L : 0L) << 38) + ((this.field2213 ? 1L : 0L) << 39);
		class101 var4 = (class101) field2175.method2930(var2);
		if (var4 != null) {
			return var4;
		}
		class136 var5 = class111.method1609(Statics.field8370, this.field2215, 0);
		if (var5 == null) {
			field2338 = true;
			return null;
		}
		if (this.field2220) {
			var5.method2602();
		}
		if (this.field2213) {
			var5.method2601();
		}
		if (this.field2218 > 0) {
			var5.method2646(this.field2218);
		} else if (this.field2219 != 0) {
			var5.method2646(1);
		}
		if (this.field2218 >= 1) {
			var5.method2599(1);
		}
		if (this.field2218 >= 2) {
			var5.method2599(16777215);
		}
		if (this.field2219 != 0) {
			var5.method2600(this.field2219 | 0xFF000000);
		}
		class101 var6 = arg0.method2200(var5, true);
		field2175.method2922(var6, var2, var6.method1459() * var6.method1435() * 4);
		return var6;
	}

	@ObfuscatedName("hf.r(Loi;Loq;S)Leu;")
	public class140 method3942(class402 arg0, class401 arg1) {
		class140 var3 = (class140) arg0.method6188(arg1, this.field2287, false, this.field2275);
		field2338 = var3 == null;
		return var3;
	}

	@ObfuscatedName("hf.v(Loi;Loq;I)Laac;")
	public class733 method3943(class402 arg0, class401 arg1) {
		class733 var3 = arg0.method6163(arg1, this.field2287);
		field2338 = var3 == null;
		return var3;
	}

	@ObfuscatedName("hf.o(Ldh;ILaof;Laov;Laod;Laon;Laos;Lem;Lep;Laaq;Lxg;I)Ldo;")
	public class116 method3944(class133 arg0, int arg1, class1089 arg2, class1084 arg3, class1082 arg4, class1095 arg5, class1080 arg6, class151 arg7, class147 arg8, class721 arg9, class639 arg10) {
		field2338 = false;
		if (this.field2224 == 0) {
			return null;
		} else if (this.field2224 == 1 && this.field2294 == -1) {
			return null;
		} else if (this.field2224 == 1) {
			int var12 = arg1;
			if (arg9 != null) {
				arg1 |= arg9.method14358();
			}
			long var13 = -1L;
			long[] var15 = class997.field11507;
			if (this.field2242 != null) {
				for (int var16 = 0; var16 < this.field2242.length; var16++) {
					long var17 = var13 >>> 8 ^ var15[(int) ((var13 ^ (long) (this.field2242[var16] >> 8)) & 0xFFL)];
					long var19 = var17 >>> 8 ^ var15[(int) ((var17 ^ (long) this.field2242[var16]) & 0xFFL)];
					long var21 = var19 >>> 8 ^ var15[(int) ((var19 ^ (long) (this.field2243[var16] >> 8)) & 0xFFL)];
					var13 = var21 >>> 8 ^ var15[(int) ((var21 ^ (long) this.field2243[var16]) & 0xFFL)];
				}
				arg1 |= 0x4000;
			}
			if (this.field2346 != null) {
				for (int var23 = 0; var23 < this.field2346.length; var23++) {
					long var24 = var13 >>> 8 ^ var15[(int) ((var13 ^ (long) (this.field2346[var23] >> 8)) & 0xFFL)];
					long var26 = var24 >>> 8 ^ var15[(int) ((var24 ^ (long) this.field2346[var23]) & 0xFFL)];
					long var28 = var26 >>> 8 ^ var15[(int) ((var26 ^ (long) (this.field2245[var23] >> 8)) & 0xFFL)];
					var13 = var28 >>> 8 ^ var15[(int) ((var28 ^ (long) this.field2245[var23]) & 0xFFL)];
				}
				arg1 |= 0x8000;
			}
			long var30 = (long) arg0.field1595 << 59 | (long) this.field2224 << 55 | (long) this.field2294 << 38 | var13 & 0x3FFFFFFFFFL;
			class116 var32 = (class116) field2297.method2930(var30);
			if (var32 == null || arg0.method2394(var32.method1691(), arg1) != 0) {
				if (var32 != null) {
					arg1 = arg0.method2213(arg1, var32.method1691());
				}
				class120 var33 = class120.method1931(Statics.field1707, this.field2294, 0);
				if (var33 == null) {
					field2338 = true;
					return null;
				}
				if (var33.field1372 < 13) {
					var33.method1947(2);
				}
				var32 = arg0.method2211(var33, arg1, Statics.field11802, 64, 768);
				if (this.field2242 != null) {
					for (int var34 = 0; var34 < this.field2242.length; var34++) {
						var32.method1859(this.field2242[var34], this.field2243[var34]);
					}
				}
				if (this.field2346 != null) {
					for (int var35 = 0; var35 < this.field2346.length; var35++) {
						var32.method1744(this.field2346[var35], this.field2245[var35]);
					}
				}
				field2297.method2921(var32, var30);
			}
			if (arg9 != null) {
				var32 = var32.method1773((byte) 1, arg1, true);
				arg9.method14359(var32, 0);
			}
			var32.method1690(var12);
			return var32;
		} else if (this.field2224 == 2) {
			class116 var36 = ((class260) arg4.method962(this.field2294)).method4544(arg0, arg1, arg7, arg8, arg9, this.field2363);
			if (var36 == null) {
				field2338 = true;
				return null;
			} else {
				return var36;
			}
		} else if (this.field2224 == 3) {
			if (arg10 == null) {
				return null;
			}
			class116 var37 = arg10.method10120(arg0, arg1, arg3, arg4, arg5, arg7, arg8, arg9);
			if (var37 == null) {
				field2338 = true;
				return null;
			} else {
				return var37;
			}
		} else if (this.field2224 == 4) {
			class738 var38 = (class738) arg5.method962(this.field2294);
			class116 var39 = var38.method14644(arg0, arg1, 10, arg10, arg9, 0, 0, 0, 0);
			if (var39 == null) {
				field2338 = true;
				return null;
			} else {
				return var39;
			}
		} else if (this.field2224 == 6) {
			class116 var40 = ((class260) arg4.method962(this.field2294)).method4542(arg0, arg1, arg2, arg7, arg8, arg9, null, null, null, 0, this.field2363);
			if (var40 == null) {
				field2338 = true;
				return null;
			} else {
				return var40;
			}
		} else if (this.field2224 != 7) {
			return null;
		} else if (arg10 == null) {
			return null;
		} else {
			int var41 = this.field2294 >>> 16;
			int var42 = this.field2294 & 0xFFFF;
			int var43 = this.field2298;
			class116 var44 = arg10.method10130(arg0, arg1, arg3, arg6, arg9, var41, var42, var43);
			if (var44 == null) {
				field2338 = true;
				return null;
			} else {
				return var44;
			}
		}
	}

	@ObfuscatedName("hf.s(Ldh;Ldo;Lou;IB)V")
	public void method3951(class133 arg0, class116 arg1, class419 arg2, int arg3) {
		arg1.method1689(arg2);
		class128[] var5 = arg1.method1750();
		class102[] var6 = arg1.method1765();
		if ((this.field2254 == null || this.field2254.field7804) && (var5 != null || var6 != null)) {
			this.field2254 = class634.method9962(arg3, false);
		}
		if (this.field2254 != null) {
			this.field2254.method9936(arg0, (long) arg3, var5, var6, false);
		}
	}

	@ObfuscatedName("hf.y(Laom;Laok;I)Lsv;")
	public class502 method4012(class1087 arg0, class1083 arg1) {
		if (this.field2339 == -1) {
			return null;
		}
		long var3 = ((long) this.field2342 & 0xFFFFL) << 48 | ((long) this.field2343 & 0xFFFFL) << 32 | ((long) this.field2344 & 0xFFFFL) << 16 | (long) this.field2339 & 0xFFFFL;
		class502 var5 = (class502) field2177.method2930(var3);
		if (var5 == null) {
			var5 = class568.method305(this.field2339, this.field2342, this.field2343, this.field2344, arg0, arg1);
			field2177.method2921(var5, var3);
		}
		return var5;
	}

	@ObfuscatedName("ack.q(IB)V")
	public static void method14950(int arg0) {
		Statics.field11802 = arg0;
		field2297.method2924();
	}

	@ObfuscatedName("lv.x(I)V")
	public static void method5403() {
		field2175.method2924();
		field2297.method2924();
		field2176.method2924();
		field2177.method2924();
	}

	@ObfuscatedName("sp.b(II)V")
	public static void method8274(int arg0) {
		field2175.method2923(arg0);
		field2297.method2923(arg0);
		field2176.method2923(arg0);
		field2177.method2923(arg0);
	}

	@ObfuscatedName("kh.h(I)V")
	public static void method5204() {
		field2175.method2928();
		field2297.method2928();
		field2176.method2928();
		field2177.method2928();
	}

	@ObfuscatedName("jh.a(B)V")
	public static void method5075() {
		Statics.field11725 = new class225[Statics.field7085.method6898()];
		Statics.field8424 = new boolean[Statics.field7085.method6898()];
	}

	@ObfuscatedName("hf.g(I)V")
	public void method3976() {
		this.field2290 = null;
		this.field2291 = null;
		this.field2292 = null;
		this.field2293 = null;
		this.field2244 = null;
		this.field2233 = null;
		this.field2296 = null;
		this.field2280 = null;
		this.field2362 = null;
		this.field2178 = null;
		this.field2249 = null;
		this.field2299 = null;
		this.field2322 = null;
		this.field2303 = null;
		this.field2353 = 0;
		this.field2304 = null;
		this.field2247 = null;
		this.field2354 = 0;
		this.field2306 = null;
		this.field2307 = null;
		this.field2160 = 0;
		this.field2308 = null;
		this.field2309 = null;
		this.field2289 = 0;
		this.field2310 = null;
		this.field2311 = null;
		this.field2357 = 0;
		this.field2312 = null;
		this.field2313 = null;
		this.field2358 = 0;
		this.field2314 = null;
		this.field2315 = null;
		this.field2225 = null;
		this.field2317 = null;
		this.field2318 = null;
		this.field2295 = null;
		this.field2320 = null;
		this.field2355 = null;
		this.field2232 = null;
		this.field2323 = null;
		this.field2347 = null;
		this.field2325 = null;
		this.field2326 = null;
		this.field2327 = null;
		this.field2328 = null;
		this.field2329 = null;
		this.field2330 = null;
		this.field2359 = null;
		this.field2332 = null;
		this.field2352 = -1;
	}

	@ObfuscatedName("hf.i(ILjava/lang/String;B)V")
	public void method3947(int arg0, String arg1) {
		if (this.field2321 == null || this.field2321.length <= arg0) {
			String[] var3 = new String[arg0 + 1];
			if (this.field2321 != null) {
				for (int var4 = 0; var4 < this.field2321.length; var4++) {
					var3[var4] = this.field2321[var4];
				}
			}
			this.field2321 = var3;
		}
		this.field2321[arg0] = arg1;
	}

	@ObfuscatedName("hf.j(III)V")
	public void method3948(int arg0, int arg1) {
		if (this.field2282 == null || this.field2282.length <= arg0) {
			int[] var3 = new int[arg0 + 1];
			if (this.field2282 != null) {
				for (int var4 = 0; var4 < this.field2282.length; var4++) {
					var3[var4] = this.field2282[var4];
				}
				for (int var5 = this.field2282.length; var5 < arg0; var5++) {
					var3[var5] = -1;
				}
			}
			this.field2282 = var3;
		}
		this.field2282[arg0] = arg1;
	}

	@ObfuscatedName("hf.t(Ldh;B)Lhx;")
	public class224 method3970(class133 arg0) {
		long var2 = (long) this.field2158 << 32 | (long) this.field2183 & 0xFFFFFFFFL;
		class224 var4 = (class224) field2176.method2930(var2);
		if (var4 != null) {
			if (this.field2215 != var4.field2148) {
				field2176.method2957(var2);
				var4 = null;
			}
			if (var4 != null) {
				return var4;
			}
		}
		class136 var5 = class111.method1609(Statics.field8370, this.field2215, 0);
		if (var5 == null) {
			return null;
		}
		int var6 = var5.method2596();
		int var7 = var5.method2597();
		int[] var8 = new int[var7];
		int[] var9 = new int[var7];
		for (int var10 = 0; var10 < var5.method2631(); var10++) {
			int var11 = 0;
			int var12 = var5.method2639();
			for (int var13 = 0; var13 < var5.method2639(); var13++) {
				if (var5.method2617(var13, var10) != 0) {
					var11 = var13;
					break;
				}
			}
			for (int var14 = var5.method2639() - 1; var14 >= var11; var14--) {
				if (var5.method2617(var14, var10) != 0) {
					var12 = var14 + 1;
					break;
				}
			}
			var8[var10 + var5.method2593()] = var11 + var5.method2591();
			var9[var10 + var5.method2593()] = var12 - var11;
		}
		class103 var15 = arg0.method2205(var6, var7, var8, var9);
		if (var15 == null) {
			return null;
		} else {
			class224 var16 = new class224(var6, var7, var9, var8, var15, this.field2215);
			field2176.method2921(var16, var2);
			return var16;
		}
	}

	@ObfuscatedName("hf.ae(III)I")
	public int method3950(int arg0, int arg1) {
		if (this.field2333 == null) {
			return arg1;
		} else {
			class988 var3 = (class988) this.field2333.method14495((long) arg0);
			return var3 == null ? arg1 : var3.field11442;
		}
	}

	@ObfuscatedName("hf.ag(ILjava/lang/String;I)Ljava/lang/String;")
	public String method3975(int arg0, String arg1) {
		if (this.field2333 == null) {
			return arg1;
		} else {
			class984 var3 = (class984) this.field2333.method14495((long) arg0);
			return var3 == null ? arg1 : (String) var3.field11436;
		}
	}

	@ObfuscatedName("hf.ah(III)V")
	public void method3952(int arg0, int arg1) {
		if (this.field2333 == null) {
			this.field2333 = new class729(16);
			this.field2333.method14501(new class988(arg1), (long) arg0);
			return;
		}
		class988 var3 = (class988) this.field2333.method14495((long) arg0);
		if (var3 == null) {
			this.field2333.method14501(new class988(arg1), (long) arg0);
		} else {
			var3.field11442 = arg1;
		}
	}

	@ObfuscatedName("hf.al(ILjava/lang/String;I)V")
	public void method4003(int arg0, String arg1) {
		if (this.field2333 == null) {
			this.field2333 = new class729(16);
			this.field2333.method14501(new class984(arg1), (long) arg0);
			return;
		}
		class984 var3 = (class984) this.field2333.method14495((long) arg0);
		if (var3 != null) {
			var3.method8440();
		}
		this.field2333.method14501(new class984(arg1), (long) arg0);
	}

	@ObfuscatedName("hf.ac(II)V")
	public void method3954(int arg0) {
		if (this.field2333 != null) {
			class532 var2 = this.field2333.method14495((long) arg0);
			if (var2 != null) {
				var2.method8440();
			}
		}
	}

	@ObfuscatedName("hf.ai(ISSI)V")
	public void method3964(int arg0, short arg1, short arg2) {
		if (arg0 >= 5) {
			return;
		}
		if (this.field2242 == null) {
			this.field2242 = new short[5];
			this.field2243 = new short[5];
		}
		this.field2242[arg0] = arg1;
		this.field2243[arg0] = arg2;
	}

	@ObfuscatedName("hf.aw(ISSI)V")
	public void method3993(int arg0, short arg1, short arg2) {
		if (arg0 >= 5) {
			return;
		}
		if (this.field2346 == null) {
			this.field2346 = new short[5];
			this.field2245 = new short[5];
		}
		this.field2346[arg0] = arg1;
		this.field2245[arg0] = arg2;
	}
}
