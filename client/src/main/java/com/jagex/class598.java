package com.jagex;

import com.jagex.deob.ObfuscatedName;
import com.jagex.deob.Statics;
import java.util.Arrays;

@ObfuscatedName("vd")
public class class598 implements class53 {

	@ObfuscatedName("vd.n")
	public static short[] field7501 = new short[256];

	@ObfuscatedName("vd.m")
	public int field7450;

	@ObfuscatedName("vd.k")
	public class599 field7451;

	@ObfuscatedName("vd.f")
	public class51 field7452;

	@ObfuscatedName("vd.w")
	public byte[] field7513;

	@ObfuscatedName("vd.l")
	public int[][] field7454;

	@ObfuscatedName("vd.u")
	public String field7517 = "null";

	@ObfuscatedName("vd.z")
	public short[] field7456;

	@ObfuscatedName("vd.p")
	public short[] field7457;

	@ObfuscatedName("vd.d")
	public byte[] field7458;

	@ObfuscatedName("vd.c")
	public short[] field7459;

	@ObfuscatedName("vd.r")
	public short[] field7489;

	@ObfuscatedName("vd.v")
	public byte[] field7461;

	@ObfuscatedName("vd.o")
	public byte[] field7462;

	@ObfuscatedName("vd.s")
	public byte field7469;

	@ObfuscatedName("vd.y")
	public byte field7464;

	@ObfuscatedName("vd.q")
	public byte field7465;

	@ObfuscatedName("vd.x")
	public byte field7493 = 0;

	@ObfuscatedName("vd.b")
	public int field7499 = 1;

	@ObfuscatedName("vd.h")
	public int field7492 = 1;

	@ObfuscatedName("vd.a")
	public int field7467 = 2;

	@ObfuscatedName("vd.g")
	public int field7470 = -1;

	@ObfuscatedName("vd.i")
	public byte field7471 = 0;

	@ObfuscatedName("vd.j")
	public int field7472 = -1;

	@ObfuscatedName("vd.t")
	public boolean field7473 = false;

	@ObfuscatedName("vd.ae")
	public int field7474 = -1;

	@ObfuscatedName("vd.ag")
	public int field7475 = 960;

	@ObfuscatedName("vd.ah")
	public int field7476 = 0;

	@ObfuscatedName("vd.al")
	public int[] field7477 = null;

	@ObfuscatedName("vd.ac")
	public int[] field7478 = null;

	@ObfuscatedName("vd.ai")
	public boolean field7479 = false;

	@ObfuscatedName("vd.aw")
	public int field7480 = 64;

	@ObfuscatedName("vd.as")
	public int field7481 = 0;

	@ObfuscatedName("vd.at")
	public int field7494 = 0;

	@ObfuscatedName("vd.ad")
	public String[] field7483;

	@ObfuscatedName("vd.am")
	public int[] field7497;

	@ObfuscatedName("vd.au")
	public int field7485 = -1;

	@ObfuscatedName("vd.ar")
	public int field7486 = -1;

	@ObfuscatedName("vd.ap")
	public boolean field7487 = false;

	@ObfuscatedName("vd.aq")
	public int field7488 = 0;

	@ObfuscatedName("vd.ax")
	public boolean field7504 = false;

	@ObfuscatedName("vd.av")
	public boolean field7490 = false;

	@ObfuscatedName("vd.ao")
	public boolean field7491 = true;

	@ObfuscatedName("vd.aj")
	public int field7460 = 128;

	@ObfuscatedName("vd.ay")
	public int field7527 = 128;

	@ObfuscatedName("vd.ab")
	public int field7528 = 128;

	@ObfuscatedName("vd.az")
	public int field7495 = 0;

	@ObfuscatedName("vd.aa")
	public int field7496 = 0;

	@ObfuscatedName("vd.af")
	public int field7498 = 0;

	@ObfuscatedName("vd.ak")
	public int field7522 = 0;

	@ObfuscatedName("vd.an")
	public int field7449 = 0;

	@ObfuscatedName("vd.bf")
	public int field7500 = 0;

	@ObfuscatedName("vd.bl")
	public boolean field7502 = false;

	@ObfuscatedName("vd.bk")
	public boolean field7455 = false;

	@ObfuscatedName("vd.bh")
	public int field7503 = -1;

	@ObfuscatedName("vd.bx")
	public int field7448 = 0;

	@ObfuscatedName("vd.bd")
	public int[] field7505;

	@ObfuscatedName("vd.bc")
	public int field7506 = -1;

	@ObfuscatedName("vd.bi")
	public int field7507 = -1;

	@ObfuscatedName("vd.bn")
	public int field7508 = -1;

	@ObfuscatedName("vd.bt")
	public int field7509 = 0;

	@ObfuscatedName("vd.bq")
	public int field7510 = 0;

	@ObfuscatedName("vd.bm")
	public int field7511 = 255;

	@ObfuscatedName("vd.bb")
	public int field7512 = 0;

	@ObfuscatedName("vd.be")
	public int field7466 = 0;

	@ObfuscatedName("vd.by")
	public int[] field7514;

	@ObfuscatedName("vd.bu")
	public boolean field7515 = true;

	@ObfuscatedName("vd.bw")
	public boolean field7516 = false;

	@ObfuscatedName("vd.bo")
	public boolean field7453 = true;

	@ObfuscatedName("vd.bz")
	public boolean field7518 = false;

	@ObfuscatedName("vd.bv")
	public class729 field7519;

	@ObfuscatedName("vd.br")
	public boolean field7520 = false;

	@ObfuscatedName("vd.bg")
	public int[] field7484;

	@ObfuscatedName("vd.ba")
	public int field7521 = 256;

	@ObfuscatedName("vd.bp")
	public int field7523 = 256;

	@ObfuscatedName("vd.bj")
	public boolean field7524 = false;

	@ObfuscatedName("vd.bs")
	public boolean field7525 = false;

	@ObfuscatedName("vd.cl")
	public class596 field7526 = class596.field7439;

	@ObfuscatedName("vd.cg")
	public class416 field7468;

	public class598(int arg0, class599 arg1, class51 arg2) {
		this.field7450 = arg0;
		this.field7451 = arg1;
		this.field7452 = arg2;
		this.field7483 = (String[]) this.field7451.field7534.clone();
	}

	@ObfuscatedName("vd.e(Lalw;B)V")
	public void method976(class997 arg0) {
		while (true) {
			int var2 = arg0.method17904();
			if (var2 == 0) {
				return;
			}
			this.method9502(arg0, var2);
		}
	}

	@ObfuscatedName("vd.n(I)V")
	public void method975() {
		this.method9472();
		if (this.field7455) {
			this.field7467 = 0;
		}
		if (!this.field7451.field7531 && this.field7518) {
			this.field7483 = null;
			this.field7484 = null;
		}
	}

	@ObfuscatedName("vd.u(Lalw;II)V")
	public void method9502(class997 arg0, int arg1) {
		if (arg1 == 1) {
			int var3 = arg0.method17904();
			this.field7513 = new byte[var3];
			this.field7454 = new int[var3][];
			for (int var4 = 0; var4 < var3; var4++) {
				this.field7513[var4] = arg0.method17905();
				int var5 = arg0.method17904();
				this.field7454[var4] = new int[var5];
				for (int var6 = 0; var6 < var5; var6++) {
					this.field7454[var4][var6] = arg0.method17927();
				}
			}
		} else if (arg1 == 2) {
			this.field7517 = arg0.method17918().intern();
		} else if (arg1 == 14) {
			this.field7499 = arg0.method17904();
		} else if (arg1 == 15) {
			this.field7492 = arg0.method17904();
		} else if (arg1 == 17) {
			this.field7467 = 0;
		} else if (arg1 != 18) {
			if (arg1 == 19) {
				this.field7470 = arg0.method17904();
			} else if (arg1 == 21) {
				this.field7471 = 1;
			} else if (arg1 == 22) {
				this.field7473 = true;
			} else if (arg1 == 23) {
				this.field7474 = 1;
			} else if (arg1 == 24) {
				int var7 = arg0.method17927();
				if (var7 != -1) {
					this.field7477 = new int[] { var7 };
				}
			} else if (arg1 == 27) {
				this.field7467 = 1;
			} else if (arg1 == 28) {
				this.field7480 = arg0.method17904() << 2;
			} else if (arg1 == 29) {
				this.field7481 = arg0.method17905();
			} else if (arg1 == 39) {
				this.field7494 = arg0.method17905();
			} else if (arg1 >= 30 && arg1 < 35) {
				this.field7483[arg1 - 30] = arg0.method17918().intern();
			} else if (arg1 == 40) {
				int var8 = arg0.method17904();
				this.field7456 = new short[var8];
				this.field7457 = new short[var8];
				for (int var9 = 0; var9 < var8; var9++) {
					this.field7456[var9] = (short) arg0.method17906();
					this.field7457[var9] = (short) arg0.method17906();
				}
			} else if (arg1 == 41) {
				int var10 = arg0.method17904();
				this.field7459 = new short[var10];
				this.field7489 = new short[var10];
				for (int var11 = 0; var11 < var10; var11++) {
					this.field7459[var11] = (short) arg0.method17906();
					this.field7489[var11] = (short) arg0.method17906();
				}
			} else if (arg1 == 42) {
				int var12 = arg0.method17904();
				this.field7458 = new byte[var12];
				for (int var13 = 0; var13 < var12; var13++) {
					this.field7458[var13] = arg0.method17905();
				}
			} else if (arg1 == 44) {
				int var14 = arg0.method17906();
				int var15 = 0;
				for (int var16 = var14; var16 > 0; var16 >>= 0x1) {
					var15++;
				}
				this.field7461 = new byte[var15];
				byte var17 = 0;
				for (int var18 = 0; var18 < var15; var18++) {
					if ((var14 & 0x1 << var18) > 0) {
						this.field7461[var18] = var17++;
					} else {
						this.field7461[var18] = -1;
					}
				}
			} else if (arg1 == 45) {
				int var19 = arg0.method17906();
				int var20 = 0;
				for (int var21 = var19; var21 > 0; var21 >>= 0x1) {
					var20++;
				}
				this.field7462 = new byte[var20];
				byte var22 = 0;
				for (int var23 = 0; var23 < var20; var23++) {
					if ((var19 & 0x1 << var23) > 0) {
						this.field7462[var23] = var22++;
					} else {
						this.field7462[var23] = -1;
					}
				}
			} else if (arg1 == 62) {
				this.field7490 = true;
			} else if (arg1 == 64) {
				this.field7491 = false;
			} else if (arg1 == 65) {
				this.field7460 = arg0.method17906();
			} else if (arg1 == 66) {
				this.field7527 = arg0.method17906();
			} else if (arg1 == 67) {
				this.field7528 = arg0.method17906();
			} else if (arg1 == 69) {
				arg0.method17904();
			} else if (arg1 == 70) {
				this.field7495 = arg0.method17973() << 2;
			} else if (arg1 == 71) {
				this.field7496 = arg0.method17973() << 2;
			} else if (arg1 == 72) {
				this.field7498 = arg0.method17973() << 2;
			} else if (arg1 == 73) {
				this.field7502 = true;
			} else if (arg1 == 74) {
				this.field7455 = true;
			} else if (arg1 == 75) {
				this.field7503 = arg0.method17904();
			} else if (arg1 == 77 || arg1 == 92) {
				this.field7506 = arg0.method17906();
				if (this.field7506 == 65535) {
					this.field7506 = -1;
				}
				this.field7507 = arg0.method17906();
				if (this.field7507 == 65535) {
					this.field7507 = -1;
				}
				int var38 = -1;
				if (arg1 == 92) {
					var38 = arg0.method17927();
				}
				int var39 = arg0.method18123();
				this.field7505 = new int[var39 + 2];
				for (int var40 = 0; var40 <= var39; var40++) {
					this.field7505[var40] = arg0.method17927();
				}
				this.field7505[var39 + 1] = var38;
			} else if (arg1 == 78) {
				this.field7508 = arg0.method17906();
				this.field7509 = arg0.method17904();
			} else if (arg1 == 79) {
				this.field7512 = arg0.method17906();
				this.field7466 = arg0.method17906();
				this.field7509 = arg0.method17904();
				int var24 = arg0.method17904();
				this.field7514 = new int[var24];
				for (int var25 = 0; var25 < var24; var25++) {
					this.field7514[var25] = arg0.method17906();
				}
			} else if (arg1 == 81) {
				this.field7471 = 2;
				this.field7472 = arg0.method17904() * 256;
			} else if (arg1 == 82) {
				this.field7516 = true;
			} else if (arg1 == 88) {
				this.field7453 = false;
			} else if (arg1 == 89) {
				this.field7515 = false;
			} else if (arg1 == 91) {
				this.field7518 = true;
			} else if (arg1 == 93) {
				this.field7471 = 3;
				this.field7472 = arg0.method17906();
			} else if (arg1 == 94) {
				this.field7471 = 4;
			} else if (arg1 == 95) {
				this.field7471 = 5;
				this.field7472 = arg0.method17973();
			} else if (arg1 == 97) {
				this.field7487 = true;
			} else if (arg1 == 98) {
				this.field7520 = true;
			} else if (arg1 == 99 || arg1 == 100) {
				arg0.method17904();
				arg0.method17906();
			} else if (arg1 == 101) {
				this.field7488 = arg0.method17904();
			} else if (arg1 == 102) {
				this.field7486 = arg0.method17906();
			} else if (arg1 == 103) {
				this.field7474 = 0;
			} else if (arg1 == 104) {
				this.field7511 = arg0.method17904();
			} else if (arg1 == 105) {
				this.field7504 = true;
			} else if (arg1 == 106) {
				int var26 = arg0.method17904();
				int var27 = 0;
				this.field7477 = new int[var26];
				this.field7478 = new int[var26];
				for (int var28 = 0; var28 < var26; var28++) {
					this.field7477[var28] = arg0.method17927();
					var27 += this.field7478[var28] = arg0.method17904();
				}
				for (int var29 = 0; var29 < var26; var29++) {
					this.field7478[var29] = this.field7478[var29] * 65535 / var27;
				}
			} else if (arg1 == 107) {
				this.field7485 = arg0.method17906();
			} else if (arg1 >= 150 && arg1 < 155) {
				this.field7483[arg1 - 150] = arg0.method17918().intern();
				if (!this.field7451.field7531) {
					this.field7483[arg1 - 150] = null;
				}
			} else if (arg1 == 160) {
				int var30 = arg0.method17904();
				this.field7484 = new int[var30];
				for (int var31 = 0; var31 < var30; var31++) {
					this.field7484[var31] = arg0.method17906();
				}
			} else if (arg1 == 162) {
				this.field7471 = 3;
				this.field7472 = arg0.method17910();
			} else if (arg1 == 163) {
				this.field7469 = arg0.method17905();
				this.field7464 = arg0.method17905();
				this.field7465 = arg0.method17905();
				this.field7493 = arg0.method17905();
			} else if (arg1 == 164) {
				this.field7522 = arg0.method17973();
			} else if (arg1 == 165) {
				this.field7449 = arg0.method17973();
			} else if (arg1 == 166) {
				this.field7500 = arg0.method17973();
			} else if (arg1 == 167) {
				this.field7448 = arg0.method17906();
			} else if (arg1 != 168 && arg1 != 169) {
				if (arg1 == 170) {
					this.field7475 = arg0.method18123();
				} else if (arg1 == 171) {
					this.field7476 = arg0.method18123();
				} else if (arg1 == 173) {
					this.field7521 = arg0.method17906();
					this.field7523 = arg0.method17906();
				} else if (arg1 == 177) {
					this.field7524 = true;
				} else if (arg1 == 178) {
					this.field7510 = arg0.method17904();
				} else if (arg1 == 186) {
					this.field7526 = (class596) class686.method1897(Statics.method9002(), arg0.method17904());
				} else if (arg1 != 188) {
					if (arg1 == 189) {
						this.field7525 = true;
					} else if (arg1 >= 190 && arg1 < 196) {
						if (this.field7497 == null) {
							this.field7497 = new int[6];
							Arrays.fill(this.field7497, -1);
						}
						this.field7497[arg1 - 190] = arg0.method17906();
					} else if (arg1 == 196) {
						class686.method1897(class608.method208(), arg0.method17904());
					} else if (arg1 == 197) {
						class686.method1897(class607.method10203(), arg0.method17904());
					} else if (arg1 != 198 && arg1 != 199) {
						if (arg1 == 200) {
							this.field7479 = true;
						} else if (arg1 == 201) {
							this.field7468 = new class416();
							this.field7468.field4252 = arg0.method17922();
							this.field7468.field4253 = arg0.method17922();
							this.field7468.field4248 = arg0.method17922();
							this.field7468.field4249 = arg0.method17922();
							this.field7468.field4250 = arg0.method17922();
							this.field7468.field4251 = arg0.method17922();
						} else if (arg1 == 249) {
							int var32 = arg0.method17904();
							if (this.field7519 == null) {
								int var33 = class783.method16657(var32);
								this.field7519 = new class729(var33);
							}
							for (int var34 = 0; var34 < var32; var34++) {
								boolean var35 = arg0.method17904() == 1;
								int var36 = arg0.method17908();
								class532 var37;
								if (var35) {
									var37 = new class984(arg0.method17918().intern());
								} else {
									var37 = new class988(arg0.method17910());
								}
								this.field7519.method14501(var37, (long) var36);
							}
						}
					}
				}
			}
		}
	}

	@ObfuscatedName("vd.z(I)V")
	public void method9472() {
		if (this.field7470 == -1) {
			this.field7470 = 0;
			if (this.field7513 != null && this.field7513.length == 1 && class601.field7563.field7562 == this.field7513[0]) {
				this.field7470 = 1;
			}
			for (int var1 = 0; var1 < 5; var1++) {
				if (this.field7483[var1] != null) {
					this.field7470 = 1;
					break;
				}
			}
		}
		if (this.field7503 == -1) {
			this.field7503 = this.field7467 == 0 ? 0 : 1;
		}
		if (this.method9504() || this.field7520 || this.field7505 != null) {
			this.field7524 = true;
		}
		if (this.field7470 <= 0 && class596.field7438 == this.field7526) {
		}
	}

	@ObfuscatedName("vd.p(IB)Z")
	public final boolean method9473(int arg0) {
		if (this.field7454 == null) {
			return true;
		}
		boolean var2 = true;
		for (int var3 = 0; var3 < this.field7513.length; var3++) {
			if (this.field7513[var3] == arg0) {
				for (int var4 = 0; var4 < this.field7454[var3].length; var4++) {
					if (!this.field7451.method9525(this.field7454[var3][var4])) {
						var2 = false;
					}
				}
			}
		}
		return var2;
	}

	@ObfuscatedName("vd.d(Lxe;S)Z")
	public final boolean method9489(class650 arg0) {
		if (this.field7454 == null) {
			return true;
		}
		boolean var2 = true;
		for (int var3 = 0; var3 < this.field7454.length; var3++) {
			for (int var4 = 0; var4 < this.field7454[var3].length; var4++) {
				boolean var5 = this.field7451.method9525(this.field7454[var3][var4]);
				var2 &= var5;
				if (!var5) {
					arg0.field7958 = this.field7454[var3][var4];
				}
			}
		}
		return var2;
	}

	@ObfuscatedName("vd.c(Ldh;IIILcb;Lcb;IIIZLvp;B)Labg;")
	public final synchronized class746 method9475(class133 arg0, int arg1, int arg2, int arg3, class104 arg4, class104 arg5, int arg6, int arg7, int arg8, boolean arg9, class597 arg10) {
		if (class601.method14238(arg2)) {
			arg2 = class601.field7548.field7562;
		}
		long var12 = (long) ((this.field7450 << 10) + (arg2 << 3) + arg3);
		long var14 = var12 | (long) (arg0.field1595 << 29);
		if (arg10 != null) {
			var14 |= arg10.field7444 << 32;
		}
		int var16 = arg1;
		if (this.field7471 == 3) {
			var16 = arg1 | 0x7;
		} else {
			if (this.field7471 != 0 || this.field7449 != 0) {
				var16 = arg1 | 0x2;
			}
			if (this.field7522 != 0) {
				var16 |= 0x1;
			}
			if (this.field7500 != 0) {
				var16 |= 0x4;
			}
		}
		if (arg9) {
			var16 |= 0x40000;
		}
		boolean var17 = this.field7471 != 0 && (arg4 != null || arg5 != null);
		boolean var18 = this.field7522 != 0 || this.field7449 != 0 || this.field7500 != 0;
		class162 var19 = this.field7451.field7533;
		class746 var20;
		synchronized (this.field7451.field7533) {
			var20 = (class746) this.field7451.field7533.method2930(var14);
		}
		class116 var22 = (class116) (var20 == null ? null : var20.field8737);
		class1175 var23 = null;
		if (var22 != null && arg0.method2394(var22.method1691(), var16) == 0) {
			var23 = (class1175) var20.field8738;
			if (arg9 && var23 == null && !var17 && !var18) {
				var23 = (class1175) (var20.field8738 = var22.method1726(null));
			}
		} else {
			if (var22 != null) {
				var16 = arg0.method2213(var16, var22.method1691());
			}
			int var24 = var16;
			if (class601.field7563.field7562 == arg2 && arg3 > 3) {
				var24 = var16 | 0x5;
			}
			var22 = this.method9476(arg0, var24, arg2, arg3, arg10);
			if (var22 == null) {
				return null;
			}
			if (class601.field7563.field7562 == arg2 && arg3 > 3) {
				var22.method1693(2048);
			}
			if (arg9 && !var17 && !var18) {
				var23 = var22.method1726(null);
			}
			var22.method1690(var16);
			class746 var25 = new class746(var22, var23);
			class162 var26 = this.field7451.field7533;
			synchronized (this.field7451.field7533) {
				this.field7451.field7533.method2921(var25, var14);
			}
		}
		class116 var28;
		if (var17 || var18) {
			var28 = var22.method1773((byte) 0, var16, true);
			if (var17) {
				var28.method1700(this.field7471, this.field7472, arg4, arg5, arg6, arg7, arg8);
			}
			if (var18) {
				var28.method1805(this.field7522, this.field7449, this.field7500);
			}
			if (arg9) {
				var23 = var28.method1726(null);
			}
			var28.method1690(arg1);
		} else {
			var28 = var22.method1773((byte) 0, arg1, true);
		}
		this.field7451.field7529.field8737 = var28;
		this.field7451.field7529.field8738 = var23;
		return this.field7451.field7529;
	}

	@ObfuscatedName("vd.r(Ldh;IIILcb;Lcb;IIILaaq;Lvp;B)Ldo;")
	public final synchronized class116 method9470(class133 arg0, int arg1, int arg2, int arg3, class104 arg4, class104 arg5, int arg6, int arg7, int arg8, class721 arg9, class597 arg10) {
		if (class601.method14238(arg2)) {
			arg2 = class601.field7548.field7562;
		}
		long var12 = (long) ((this.field7450 << 10) + (arg2 << 3) + arg3);
		int var14 = arg1;
		long var15 = var12 | (long) (arg0.field1595 << 29);
		if (arg10 != null) {
			var15 |= arg10.field7444 << 32;
		}
		if (arg9 != null) {
			arg1 |= arg9.method14358();
		}
		if (this.field7471 == 3) {
			arg1 |= 0x7;
		} else {
			if (this.field7471 != 0 || this.field7449 != 0) {
				arg1 |= 0x2;
			}
			if (this.field7522 != 0) {
				arg1 |= 0x1;
			}
			if (this.field7500 != 0) {
				arg1 |= 0x4;
			}
		}
		if (class601.field7563.field7562 == arg2 && arg3 > 3) {
			arg1 |= 0x5;
		}
		class162 var17 = this.field7451.field7532;
		class116 var18;
		synchronized (this.field7451.field7532) {
			var18 = (class116) this.field7451.field7532.method2930(var15);
		}
		if (var18 == null || arg0.method2394(var18.method1691(), arg1) != 0) {
			if (var18 != null) {
				arg1 = arg0.method2213(arg1, var18.method1691());
			}
			var18 = this.method9476(arg0, arg1, arg2, arg3, arg10);
			if (var18 == null) {
				return null;
			}
			class162 var20 = this.field7451.field7532;
			synchronized (this.field7451.field7532) {
				this.field7451.field7532.method2921(var18, var15);
			}
		}
		boolean var22 = false;
		if (arg9 != null) {
			var18 = var18.method1773((byte) 1, arg1, true);
			var22 = true;
			arg9.method14359(var18, arg3 & 0x3);
		}
		if (class601.field7563.field7562 == arg2 && arg3 > 3) {
			if (!var22) {
				var18 = var18.method1773((byte) 3, arg1, true);
				var22 = true;
			}
			var18.method1693(2048);
		}
		if (this.field7471 != 0) {
			if (!var22) {
				var18 = var18.method1773((byte) 3, arg1, true);
				var22 = true;
			}
			var18.method1700(this.field7471, this.field7472, arg4, arg5, arg6, arg7, arg8);
		}
		if (this.field7522 != 0 || this.field7449 != 0 || this.field7500 != 0) {
			if (!var22) {
				var18 = var18.method1773((byte) 3, arg1, true);
				var22 = true;
			}
			var18.method1805(this.field7522, this.field7449, this.field7500);
		}
		if (var22) {
			var18.method1690(var14);
		}
		return var18;
	}

	@ObfuscatedName("vd.v(Ldh;IIILvp;I)Ldo;")
	public class116 method9476(class133 arg0, int arg1, int arg2, int arg3, class597 arg4) {
		int var6 = this.field7481 + 64;
		int var7 = this.field7494 * 5 + 850;
		int var8 = arg1;
		boolean var9 = this.field7490 || class601.field7545.field7562 == arg2 && arg3 > 3;
		if (var9) {
			arg1 |= 0x10;
		}
		if (arg3 == 0) {
			if (this.field7460 != 128 || this.field7495 != 0) {
				arg1 |= 0x1;
			}
			if (this.field7528 != 128 || this.field7498 != 0) {
				arg1 |= 0x4;
			}
		} else {
			arg1 |= 0xD;
		}
		if (this.field7527 != 128 || this.field7496 != 0) {
			arg1 |= 0x2;
		}
		if (this.field7456 != null) {
			arg1 |= 0x4000;
		}
		if (this.field7459 != null) {
			arg1 |= 0x8000;
		}
		if (this.field7493 != 0) {
			arg1 |= 0x80000;
		}
		class116 var10 = null;
		if (this.field7513 != null) {
			int var11 = -1;
			for (int var12 = 0; var12 < this.field7513.length; var12++) {
				if (this.field7513[var12] == arg2) {
					var11 = var12;
					break;
				}
			}
			if (var11 == -1) {
				return null;
			}
			int[] var13 = arg4 == null || arg4.field7443 == null ? this.field7454[var11] : arg4.field7443;
			int var14 = var13.length;
			if (var14 > 0) {
				long var15 = (long) arg0.field1595;
				for (int var17 = 0; var17 < var14; var17++) {
					var15 = var15 * 67783L + (long) var13[var17];
				}
				class162 var18 = this.field7451.field7530;
				synchronized (this.field7451.field7530) {
					var10 = (class116) this.field7451.field7530.method2930(var15);
				}
				if (var10 != null) {
					if (var6 != var10.method1862()) {
						arg1 |= 0x1000;
					}
					if (var7 != var10.method1740()) {
						arg1 |= 0x2000;
					}
				}
				if (var10 == null || arg0.method2394(var10.method1691(), arg1) != 0) {
					int var20 = arg1 | 0x1F01F;
					if (var10 != null) {
						var20 = arg0.method2213(var20, var10.method1691());
					}
					class120 var21 = null;
					class120[] var22 = this.field7451.field7535;
					synchronized (this.field7451.field7535) {
						int var23 = 0;
						while (true) {
							if (var23 >= var14) {
								if (var14 > 1) {
									var21 = new class120(this.field7451.field7535, var14);
								}
								break;
							}
							byte[] var24 = this.field7451.method9530(var13[var23]);
							if (var24 == null) {
								return null;
							}
							var21 = new class120(var24);
							if (var21.field1372 < 13) {
								var21.method1947(2);
							}
							if (var14 > 1) {
								this.field7451.field7535[var23] = var21;
							}
							var23++;
						}
					}
					var10 = arg0.method2211(var21, var20, this.field7451.field7536, var6, var7);
					class162 var26 = this.field7451.field7530;
					synchronized (this.field7451.field7530) {
						this.field7451.field7530.method2921(var10, var15);
					}
				}
			}
		}
		if (var10 == null) {
			return null;
		}
		class116 var28 = var10.method1773((byte) 0, arg1, true);
		if (var6 != var10.method1862()) {
			var28.method1893(var6);
		}
		if (var7 != var10.method1740()) {
			var28.method1738(var7);
		}
		if (var9) {
			var28.method1698();
		}
		if (class601.field7548.field7562 == arg2 && arg3 > 3) {
			var28.method1694(2048);
			var28.method1805(180, 0, -180);
		}
		int var29 = arg3 & 0x3;
		if (var29 == 1) {
			var28.method1694(4096);
		} else if (var29 == 2) {
			var28.method1694(8192);
		} else if (var29 == 3) {
			var28.method1694(12288);
		}
		if (this.field7456 != null) {
			short[] var30;
			if (arg4 == null || arg4.field7442 == null) {
				var30 = this.field7457;
			} else {
				var30 = arg4.field7442;
			}
			for (int var31 = 0; var31 < this.field7456.length; var31++) {
				if (this.field7458 == null || var31 >= this.field7458.length) {
					var28.method1859(this.field7456[var31], var30[var31]);
				} else {
					var28.method1859(this.field7456[var31], field7501[this.field7458[var31] & 0xFF]);
				}
			}
		}
		if (this.field7459 != null) {
			short[] var32;
			if (arg4 == null || arg4.field7445 == null) {
				var32 = this.field7489;
			} else {
				var32 = arg4.field7445;
			}
			for (int var33 = 0; var33 < this.field7459.length; var33++) {
				var28.method1744(this.field7459[var33], var32[var33]);
			}
		}
		if (this.field7493 != 0) {
			var28.method1745(this.field7469, this.field7464, this.field7465, this.field7493 & 0xFF);
		}
		if (this.field7460 != 128 || this.field7527 != 128 || this.field7528 != 128) {
			var28.method1699(this.field7460, this.field7527, this.field7528);
		}
		if (this.field7495 != 0 || this.field7496 != 0 || this.field7498 != 0) {
			var28.method1805(this.field7495, this.field7496, this.field7498);
		}
		var28.method1690(var8);
		return var28;
	}

	@ObfuscatedName("vd.o(Lem;Lep;I)Lvd;")
	public final class598 method9477(class151 arg0, class147 arg1) {
		int var3 = -1;
		if (this.field7506 != -1) {
			class305 var4 = arg0.method694(this.field7506);
			if (var4 != null) {
				var3 = arg1.method678(var4);
			}
		} else if (this.field7507 != -1) {
			class149 var5 = arg0.method695(class455.field4594, this.field7507);
			if (var5 != null) {
				var3 = arg1.method679(var5);
			}
		}
		if (var3 >= 0 && var3 < this.field7505.length - 1) {
			return this.field7505[var3] == -1 ? null : (class598) this.field7452.method962(this.field7505[var3]);
		} else {
			int var6 = this.field7505[this.field7505.length - 1];
			return var6 == -1 ? null : (class598) this.field7452.method962(var6);
		}
	}

	@ObfuscatedName("vd.s(III)I")
	public int method9481(int arg0, int arg1) {
		if (this.field7519 == null) {
			return arg1;
		} else {
			class988 var3 = (class988) this.field7519.method14495((long) arg0);
			return var3 == null ? arg1 : var3.field11442;
		}
	}

	@ObfuscatedName("vd.y(ILjava/lang/String;B)Ljava/lang/String;")
	public String method9479(int arg0, String arg1) {
		if (this.field7519 == null) {
			return arg1;
		} else {
			class984 var3 = (class984) this.field7519.method14495((long) arg0);
			return var3 == null ? arg1 : (String) var3.field11436;
		}
	}

	@ObfuscatedName("vd.q(I)Z")
	public boolean method9480() {
		if (this.field7505 == null) {
			return this.field7508 != -1 || this.field7514 != null;
		}
		for (int var1 = 0; var1 < this.field7505.length; var1++) {
			if (this.field7505[var1] != -1) {
				class598 var2 = (class598) this.field7452.method962(this.field7505[var1]);
				if (var2.field7508 != -1 || var2.field7514 != null) {
					return true;
				}
			}
		}
		return false;
	}

	@ObfuscatedName("vd.x(B)Z")
	public boolean method9504() {
		return this.field7477 != null;
	}

	@ObfuscatedName("vd.b(I)Z")
	public boolean method9514() {
		return this.field7477 != null && this.field7477.length > 1;
	}

	@ObfuscatedName("vd.h(S)I")
	public int method9483() {
		if (this.field7477 != null) {
			if (this.field7477.length <= 1) {
				return this.field7477[0];
			}
			int var1 = (int) (Math.random() * 65535.0D);
			for (int var2 = 0; var2 < this.field7477.length; var2++) {
				if (var1 <= this.field7478[var2]) {
					return this.field7477[var2];
				}
				var1 -= this.field7478[var2];
			}
		}
		return -1;
	}

	@ObfuscatedName("vd.a(I)[I")
	public int[] method9484() {
		return this.field7477;
	}

	@ObfuscatedName("vd.g(IB)Z")
	public boolean method9493(int arg0) {
		if (this.field7477 != null && arg0 != -1) {
			for (int var2 = 0; var2 < this.field7477.length; var2++) {
				if (this.field7477[var2] == arg0) {
					return true;
				}
			}
		}
		return false;
	}

	@ObfuscatedName("vd.i(IS)I")
	public int method9486(int arg0) {
		return this.field7497 == null ? -1 : this.field7497[arg0];
	}
}
